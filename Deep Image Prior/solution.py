...
class InspectPredictions(Callback):
    ...
    def visualize_image_content(self, image_content_array, epoch, split_name):
        for image_index, image_content in enumerate((image_content_array * 255).astype(np.uint8), start=1):
            image_file_path = os.path.join(PREDICTIONS_FOLDER_PATH, "epoch_{}_sample_{}_{}.png".format(epoch + 1, image_index, split_name))
            cv2.imwrite(image_file_path, image_content)

    def on_epoch_end(self, epoch, logs=None):
        input_image_content_array, _ = next(self.batch_generator)
        predicted_image_content_array = self.model.predict_on_batch(input_image_content_array)

        self.visualize_image_content(predicted_image_content_array, epoch, split_name="prediction")
...
def run():
    ...
    model = init_model(IMAGE_SIZE, FILTERS_LIST, LEARNING_RATE, os.path.join(OPTIMAL_WEIGHTS_FOLDER_PATH, "model.png"))

    print("Initiating the batch generators ...")
    train_batch_generator, valid_batch_generator, inspection_batch_generator = init_batch_generators(IMAGE_URL, IMAGE_SIZE, MEAN, STANDARD_DEVIATION, OUTPUT_FOLDER_PATH)

    print("Performing the training procedure ...")
    modelcheckpoint_callback = ModelCheckpoint(os.path.join(OPTIMAL_WEIGHTS_FOLDER_PATH, "model.h5"),
        monitor="val_loss", save_best_only=True, save_weights_only=False, verbose=1)  # NB: Theoretically,ground truth might be unavailable.
    ...
    model.fit_generator(generator=train_batch_generator,
            ...
            validation_steps=1,
            callbacks=[modelcheckpoint_callback, inspectloss_callback, inspectpredictions_callback],
