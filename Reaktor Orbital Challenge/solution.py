...
def run():
    ...
    for row_index in range(1, file_content.shape[0] - 1):
        satellite_coordinates_dict[file_content[row_index, 0]] = get_coordinates(file_content[row_index, 1:-1])

    # Read the coordinates of the starting and ending points
    starting_point_coordinates = get_coordinates(file_content[-1, 1:3].tolist() + [0])
    ending_point_coordinates = get_coordinates(file_content[-1, 3:].tolist() + [0])
    ...
    for satellite_name, satellite_coordinates in satellite_coordinates_dict.items():
        if is_valid_connection(starting_point_coordinates, satellite_coordinates):
    ...
    for satellite_combination in combinations(satellite_coordinates_dict.keys(), 2):
        if is_valid_connection(satellite_coordinates_dict[satellite_combination[0]], satellite_coordinates_dict[satellite_combination[1]]):
            valid_next_satellites[satellite_combination[0]].append(satellite_combination[1])
            valid_next_satellites[satellite_combination[1]].append(satellite_combination[0])
    ...
    for search_depth in range(1, len(satellite_coordinates_dict.keys()) + 1):
        ...
        possible_routes = np.array(list(permutations(satellite_coordinates_dict.keys(), search_depth)))
        ...
        for index in range(0, search_depth - 1):
            ...
            for route_index, possible_route in enumerate(possible_routes):
                if possible_route[index + 1] in valid_next_satellites[possible_route[index]]:
